...
import numpy as np
from sklearn import (metrics, cross_validation, svm, grid_search,preprocessing)
...
def main():
    ...
    """
    model = svm.SVC(kernel='poly',probability=True)  # the classifier we'll use
    parameters={'degree':[1,2,3,4],'C':[0.1,0.5,1,1.5],'gamma':[0,0.5,1]}
    clf = grid_search.GridSearchCV(model,parameters,refit=False,n_jobs=-1)
    # === load data in memory === #
    print "loading data"
